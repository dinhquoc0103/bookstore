<?php
    ...
    $filterGroupNameVal = (Session::get('filter_group_name') != null) ? Session::get('filter_group_name')  : 18;
    ...
    $this->selectBoxGroup['default'] = 'Filter By Group';
    $arrGroup = $this->selectBoxGroup;
    ksort($arrGroup);    
    $selectGroup = Helper::CreateSelectBox($arrGroup, 'filter_group_name', 'filter_group_name', 'form-control', $filterGroupNameVal);
    ...
    if(!empty($this->listItems)){
        foreach($this->listItems as $item){
            ...
            $status = Helper::createStatus(
                $item['status'],
                Url::createLink('admin', 'user', 'ajaxChangeStatus', array('status' => $item['status'], 'id' => $id)), 
                $id
            );
            ...
            $linkEdit = Url::createLink('admin', 'user', 'form', ['id' => $id], 'admin/user/form/id-'. $id);
            $rowUserHtml .=   '<tr>
                            <th scope="row">
                                <input value="'.$id.'" class="checkbox" type="checkbox" name="checkBoxId[]">
                            </th>
                            <td><a href="'.$linkEdit.'" title="Edit User '.$fullName.'">'.$fullName.'</a></td>
                            <td>'.$email.'</td>
                            <td>'.$groupName.'</td>
                            <td>'.Helper::shortenText($address, 5).'</td>
                            <td>'.$status.'</td>
                            <td>
    ...
    $linkClearKeyword = Url::createLink('admin', 'user', 'clearKeyword');
...
<div class="page-body">
    ...
    <div class="card">
        ...
        <div class="card-block table-border-style">
            <form class="form-material" action="<?php echo ROOT_URL . 'admin/user/';?>" method="post" name="adminForm" id="adminForm">
                <div class="row">
